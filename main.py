...
def scrap_board_jeonju(
    ...
    while True:
        ...
        for post in trs:
            ...
            link = domain + tds[1].find('a').attrs['href'].lstrip('.')
            author = tds[2].text
            date = tds[3].text
...
# 전주시평생학습관, 전북여성교육문화센터 게시판 스크래핑
def scrap_board_jj(
        board='https://e.jeonju.go.kr/home/SubPage.html?&page=',
        title_scrap='전주시 공지사항',
        file_scrap='jeonju_noti',
        table_class='tstyle MT10',
        domain=''
...
@sched.scheduled_job('cron', day_of_week='mon-fri', hour='9,15')
def timed_job():
    global bot
    ...
    scrap_board_jeonju(url_jeonju[0], url_jeonju[1], url_jeonju[2], url_jeonju[3])
